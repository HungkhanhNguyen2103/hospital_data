...
class SendEmailController{
    ...
    async sendEmail(req,res){
            ...
            const resultVerify = await axios.get(process.env.API_VERIFY_TOKEN + result.data.accessToken)
        // return res.json(resultVerify.data)
        
            if (resultVerify.data.student.isActive) {
                const scores = await axios.get(process.env.API_SCORES_STUDENT + resultVerify.data.student.code)
        ...
        setInterval(  async () => {
            const updateScore = await axios.get(process.env.API_SEND_MAIL + 'update_scores/' + result.data.accessToken)
            if(updateScore.data !== 0){
                const send = await axios.get(process.env.API_SEND_MAIL + 'scores/' + result.data.accessToken)
    ...
    async requestScore (req , res){
      ...
      const resultVerify = await axios.get(process.env.API_VERIFY_TOKEN + accessToken)
      const scores = await axios.get(process.env.API_SCORES_STUDENT + resultVerify.data.student.code)
      return res.json(scores.data)
    }
    ...
    async checkUpdateScore (req , res){
        ...
        const resultVerify = await axios.get(process.env.API_VERIFY_TOKEN + accessToken)
        const scores = await axios.get(process.env.API_SCORES_STUDENT + '/check_update/' + resultVerify.data.student.code)
        return res.json(scores.data)
      }
